...
-- Corrected / repair-safe migration.
-- Run this file after protect_admin_self_lockout.sql.
-- It is safe to run whether the earlier add_owner_admin_team.sql attempt rolled
-- back completely or stopped after applying some earlier statements.
...
-- Promote the oldest existing admin to the single protected Owner role if an
-- Owner has not already been established.
...
-- CURRENT_ROLE is a PostgreSQL keyword. Quoting the output identifier keeps the
-- JSON/RPC field name as current_role for the React UI without a syntax error.
...
-- ---------------------------------------------------------------------------
-- Staff accounts cannot self-delete even while suspended/banned.
-- ---------------------------------------------------------------------------
