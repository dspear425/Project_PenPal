...
export default function FeedbackShortcut() {
  ...
  async function submit(event: React.FormEvent) {
    ...
    try {
      ...
      setBody('')
      setKind('general')
      setMessage('Thanks — your feedback was sent privately to the OutKin team. You can review replies later in Help → My conversations.')
    } catch (error) {
      ...
      {open && createPortal(
        <div className="beta-feedback-overlay" onMouseDown={(event) => { if (event.target === event.currentTarget && !working) setOpen(false) }}>
          <section className="beta-feedback-panel" role="dialog" aria-modal="true" aria-labelledby="feedback-title">
            <header>
              <div><p className="eyebrow">Early access</p><h2 id="feedback-title">Tell us what you notice.</h2><p>Report a rough edge, share an idea, or tell us what is working well. Feedback is private between your account and OutKin staff.</p></div>
