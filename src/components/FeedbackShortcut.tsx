...
export default function FeedbackShortcut() {
  ...
  async function submit(event: React.FormEvent) {
    ...
    try {
      ...
      const ticketSubject = (`Feedback: ${kindLabel(kind)}${cleanSubject ? ` — ${cleanSubject}` : ''}`).slice(0, 120)
      ...
      const firstMessage = [
        ...
        'Please do not include passwords, authentication tokens, or private mailing addresses in feedback.',
      ].join('\n')
      ...
      setBody('')
      setKind('general')
      setMessage('Thanks — your feedback was sent privately to the Project PenPal team. You can review replies later in Help → My conversations.')
    } catch (error) {
  ...
  if (!signedIn || !target) return null
    ...
    <>
      ...
      {open && createPortal(
        <div className="beta-feedback-overlay" onMouseDown={(event) => { if (event.target === event.currentTarget && !working) setOpen(false) }}>
          <section className="beta-feedback-panel" role="dialog" aria-modal="true" aria-labelledby="feedback-title">
            <header>
              <div><p className="eyebrow">Early access</p><h2 id="feedback-title">Tell us what you notice.</h2><p>Report a rough edge, share an idea, or tell us what is working well. Feedback is private between your account and Project PenPal staff.</p></div>
            ...
            <form onSubmit={submit} className="beta-feedback-form">
              ...
              <div className="beta-feedback-actions"><button className="primary" disabled={working || !body.trim()}>{working ? 'Sending…' : 'Send feedback'}</button><button className="secondary" type="button" onClick={() => setOpen(false)} disabled={working}>Close</button></div>
            </form>
          </section>
        </div>,
        document.body,
