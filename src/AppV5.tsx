...
export default function AppV5() {
  ...
  useEffect(() => {
    ...
    const refreshCount = () => void loadIncomingRequestCount(session.user.id)
    refreshCount()

    // Keep the dashboard useful when a request arrives while the member is already signed in.
  ...
  async function loadIncomingRequestCount(userId: string) {
    ...
    if (!error) setIncomingRequestCount(count ?? 0)
  }
      ...
      <main className="page-shell">
        <section className="hero-card dashboard-card">
          ...
          <div className="actions dashboard-actions">
            ...
            <button
              className={`secondary request-dashboard-button ${incomingRequestCount > 0 ? 'has-notification' : ''}`}
              onClick={() => setMode('connections')}
              aria-label={incomingRequestCount > 0
                ? `Pen pals and requests, ${incomingRequestCount} incoming request${incomingRequestCount === 1 ? '' : 's'}`
