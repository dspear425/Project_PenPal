...
export default function AppV2() {
  ...
  useEffect(() => {
    supabase.auth.getSession().then(({ data }) => {
      ...
      if (data.session) void loadUser(data.session)
    })

    const { data: listener } = supabase.auth.onAuthStateChange((_event, nextSession) => {
      setSession(nextSession)
      if (nextSession) void loadUser(nextSession)
      else {
        setMode('welcome')
  ...
  async function loadUser(activeSession: Session) {
    ...
    try {
      // Load the public interest catalog independently so a separate profile issue
      // cannot leave the entire Interests section blank.
      ...
      // maybeSingle() lets us distinguish a missing row from a database error.
      ...
      // Self-heal a missing profile row. This is safe because RLS only allows a user
      // to insert a row whose id matches auth.uid().
      if (!profileData) {
      ...
      // Existing selected interests are optional during onboarding. If this query
      // fails, keep the rest of the form usable and show the exact reason.
      ...
      if (chosenError) {
        notices.push(`Selected interests: ${errorMessage(chosenError)}`)
        setSelectedInterests([])
      } else {
        setSelectedInterests((chosen ?? []).map((item) => Number(item.interest_id)))
      }
      ...
      <main className="page-shell">
        <section className="hero-card dashboard-card">
          ...
          <p className="hero-copy">
            Your profile is ready. Next we’ll build Discover, compatibility matching,
            pen-pal requests, and your letter inbox.
          </p>
      ...
      <main className="page-shell onboarding-shell">
        <section className="hero-card onboarding-card">
          ...
          <p className="hero-copy compact">
            We only ask for information that helps people match. Exact location and
            private contact details stay out of your public profile.
          </p>
